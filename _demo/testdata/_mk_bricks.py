...
""")
    abslen += of.write(f"^{abslen}\n\n")
    chars = lambda start: str().join(chr(c) for c in range(start, start + 51))
    abslen += of.write(str().join(chars(start) + f"        \t<- ord {start} to {start + 50}\n" for start in (180, 1200, 3920, 8810, 9490)))
    abslen += of.write('\n✚✛✜✝✞✟✠✡✢✣✤✥✦✧!✩✪✫✬✭✮✯✰✱✲✳✴✵✶✷✸✹✺✻✼✽✾✿❀❁❂❃❄❅❆❇❈❉❊❋!  <- ord 10010 to 10060, except 10024 and 10060 (!)\n')
    abslen += of.write(f"^{abslen}\n\n")
    abslen += of.write(chr(10024) + chr(10060) + ' <- ord 10024 and 10060 \n')
    abslen += of.write(
        '^Terminal-Compatibility-Problem observed. (Reliable remove: goto_row(42); 2x del)\n'
        'The characters widen the column (-> column overflow) and use two positions (-> deformed cursor positioning).\n'
        'The overflow can be uncleanly solved by "\\x1b[?7l" (DECPM: disable Auto-Warp-Mode).\n')
    abslen += of.write(f"^{abslen}\n\n")
    abslen += of.write(
        str().join((' ' * n) + "\t" + c + "\n" for n, c in enumerate(("!", '"', "§", "$", "%", "&", "/", "("))))
    abslen += of.write(f"^{abslen}\n\n")
    abslen += of.write("-blank blank tab char * 5-\n")
    abslen += of.write(str().join("  \t" + c for c in "abcde") + "\n")
    abslen += of.write(f"^{abslen}\n")
    abslen += of.write("-tab char * 5-\n")
    abslen += of.write(str().join("\t" + c for c in "vwxyz") + "\n")
    abslen += of.write(f"^{abslen}\n")
    abslen += of.write("-tab blank char tab * 3-\n")
    abslen += of.write(str().join("\t " + c + "\t" for c in "987") + "\n")
    abslen += of.write(f"^{abslen}\n")
    abslen += of.write("-tab blank tab char * 2-\n")
    abslen += of.write(str().join("\t \t" + c for c in "<>") + "\n")
    abslen += of.write(f"^{abslen}\n")
    abslen += of.write("-char tab blank * 9-\n")
    abslen += of.write(str().join(c + "\t " for c in ",.-#+´ß0") + "\n")
    abslen += of.write(f"^{abslen}\n\n")
    for aa in (r"""
    ...
    """
        ...
        abslen += of.write(b := "=" * len(max(aa.split("\n"), key=len)))
        abslen += of.write(aa + "\n")
        abslen += of.write(b + "\n")
        abslen += of.write(f"^{abslen}\n")
    abslen += of.write("\n4 newlines:\n\n\n\n-end-")
    ...
    abslen += of.write("\n32 newlines:\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n-end-")
    abslen += of.write(f"^{abslen}\n\n")
    _l = of.write(str().join("..%-2d.....\n" % i for i in range(1, 81)))
    abslen += of.write(f"{_l=}\n") + _l
    abslen += of.write(f"^{abslen}\n\n")
    abslen += of.write("\tß   \t1\t4  \$ \t\t\tEND")
    abslen += of.write(f"|< {abslen=}")
    abslen += of.write("\n\n<'''>\n")
    abslen += of.write(r'''
...
def open(file, mode='r', buffering=None, encoding=None, errors=None, newline=None, closefd=True): # known special case of open
    ...
    * On input, if newline is None, universal newlines mode is
      ...
      the _char legal values, input lines are only terminated by the given
...
eval.\ :html:`<a id=#eval><strong>Eval</strong></a>`\ (*x*) -> Char | Key | Mouse | Reply | Type[Char | Key | Mouse | Reply] :html:`<a href="!¶/wiki/eval#user-content-#eval">¶</a>`

    The central Eval function.
    Returns the input type or the object from a representational string.

    :raises:
        - ValueError if the repr-string cannot be associated.
    


*class* eval.\ :html:`<a id=#basickeycomp><strong>BasicKeyComp</strong></a>` :html:`<a href="!¶/wiki/eval#user-content-#basickeycomp">¶</a>`

    *class* eval.\ :html:`<a id=#basickeycomp-navkeys><strong>NavKeys</strong></a>` :html:`<a href="!¶/wiki/eval#user-content-#basickeycomp-navkeys">¶</a>`
    """)
    abslen += of.write(f"|< {abslen=}")
    abslen += of.write(r'''
